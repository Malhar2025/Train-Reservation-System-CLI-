...
void bookTickets(int currentPNR)
{
    ...
    while(pass > 0)
    {
        ...
        case 3:
            cout << endl;
            for (int i = 0; i < wT.waiting.size(); i++)
            {
                for (int j = 0; j < wT.waiting[i].size(); j++)
                {
                    cout << wT.waiting[i][j] << " ";
                }
                cout << endl;
        ...
        for (int j = 0; j < tD.v[i].size(); j++)
        {
            ...
            if(t != string::npos and i == row+1)
            {
                ...
                else if(connect != 1)
                {
                    ...
                    wait++;
                    info[count].push_back(tD.v[i][j]);
                    string p = to_string(stoi(bR.passengersStr) - stoi(tD.v[i][j].substr(n+1)));
                    info[count].push_back(p);
                    temp2.push_back(to_string(i));
                    break;
                }
                else
                {
                    wait++;
                    waiting.push_back(tD.v[i][0]);
                    waiting.push_back(tD.v[i][j]);
                    waiting.push_back(tD.v[i][stoi(bR.to)]);
                    wT.waiting.push_back(waiting);
                    info[count].push_back(tD.v[i][j]);
                    string p = bR.passengersStr;
                    info[count].push_back(p);
                    temp2.push_back(to_string(i));
                    break;
                }
    ...
    for(int i=0;i<updated_info.size();i++)
    {
        if(user_input == updated_info[i][0])
        {
            ...
            waits = updated_info[i][(updated_info[i].size()-1)];
            distance = d2 - d1;
        }
    }
            ...
            for(int j=0; j<tD.v[i].size(); j++)
            {
                ...
                if(t != string::npos)
                {
                    if(wait != 0)
                    {
                        ...
                        tD.v[i][j].erase();
                        int waiting_seats = stoi(bR.passengersStr) - available_seats;
                        waits = to_string(waiting_seats);
                        break;
                    }
                    if(connect == 1)
                    {
                    ...
                    else
                    {
                        ...
                        confirmed_seats = required_seats;
    ...
    for (int i = serial_num; i < serial_num + required_seats; i++)
    {
        details.push_back(bR.classType+"-"+to_string(i));
    }

    if(wait != 0)
    {
        for (int i = 0; i < stoi(waits); i++)
        {
            details.push_back("WL-" + to_string(wait_serial_num));
        }
    }
    ...
    pass++;
    currentPNR++;
    serial_num += stoi(bR.passengersStr);
    goto redirect;
}
